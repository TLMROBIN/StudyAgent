# Question-Bank Recommendation Optimization — Consensus Plan v2
...
### Team launch hints
- `$team .omx/plans/ralplan-question-bank-recommendation-analysis-v2.md`
- `omx team "Implement .omx/plans/ralplan-question-bank-recommendation-analysis-v2.md with shared question-bank post-processing, mixed-data fallback, and regression coverage"`

### Ralph launch hint
- `$ralph .omx/plans/ralplan-question-bank-recommendation-analysis-v2.md`
...
## 13) Architect Review (sequential review)

### Steelman Antithesis
A new shared post-processing layer may be unnecessary abstraction. Because PDF already has a structured bridge and DOCX already works through `rag_service`, the lower-risk move could be to keep ownership in current files and extract only a few shared helpers for metadata normalization. That would reduce module churn and avoid creating a second orchestration surface inside ingest.

### Real Tradeoff Tension
- **Boundary clarity vs rollout size**: a dedicated shared layer is architecturally cleaner, but increases refactor scope.
- **Question-item precision vs recall safety**: stricter recommendation improves quality, but only if fallback is explicit and test-backed.
- **Diagnostic richness vs compatibility simplicity**: more metadata helps debugging, but every persisted field adds preservation and maintenance cost.

### Synthesis
Keep the new layer **narrow and resource-scoped**:
- question-bank resources only
- called from existing PDF/DOCX preparation paths rather than replacing them wholesale
- no new public API contract in phase 1

This preserves the architectural win without turning the change into an ingest rewrite.

### Architect-adjusted directive
Treat `question_bank_post_processor` as a focused domain service, not a new top-level ingestion framework.

---

## 14) Critic Evaluation

### Verdict
**APPROVE**

### Approval basis
- Principles are consistent with the chosen option and non-goals.
- Alternatives are real and fairly bounded.
- Acceptance criteria are concrete enough to drive implementation and verification.
- Test plan covers the main regression axes: contract parity, mixed-data fallback, image diagnostics, metadata preservation, and serializer compatibility.
- Staffing guidance and verification path are explicit for both `ralph` and `team`.

### Critical conditions to preserve during execution
1. Do not collapse fallback and primary candidate paths back into one permissive `_is_question_row()` rule.
2. Do not add new metadata without also deciding whether it belongs in preservation keys, API schemas, or neither.
3. Do not introduce `quality_score` in the first pass.
4. Do not expand scope into textbook recommendation or TXT handling.

### Verification checklist for execution handoff
- PRD exists: `.omx/plans/prd-question-bank-recommendation-analysis.md`
- Test spec exists: `.omx/plans/test-spec-question-bank-recommendation-analysis.md`
- Consensus plan exists: `.omx/plans/ralplan-question-bank-recommendation-analysis-v2.md`
- Mixed-data fallback rule is explicit
- Metadata layering is explicit
- Team/Ralph staffing and verification path are explicit
